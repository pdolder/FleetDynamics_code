...
get_ensemble_mod <- function(train.data = NULL, model_fit = NULL, test.yrs = NULL, train.yrs = NULL, 
			     type = NULL, Closure = c(FALSE,FALSE), model_predictions = NULL, DSVM_optim = FALSE) {
...
if(type %in% c("Ensemble_lm","Ensemble_dirichlet")) {
	...
	predictions[[y]] <- predict_ensemble_with_mod(train.data = train.data, model_predictions = model_predictions, type = type, train.yrs = train.yrs, test.yrs =test.yrs[y], Closure = Closure[y])
